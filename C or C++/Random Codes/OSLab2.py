...
arrival_time = list(map(int, input().split())) #Reads Arrival Time
burst_time = list(map(int, input().split())) #Reads Burst Time

# Initialising and Declaring lists for the processes
...
remaining_time = burst_time.copy() #RT = BT initially

# Some error handling and taking input for algo 4 and algo 5
if (algo == 4):
    priority_val = list(map(int, input().split())) #Reads the priority Values
    if (len(priority_val) != proc_count):
    ...
    while len(ready_queue) > 0:
        ...
        if t in arrival_time or t == completion_time[p] and count != 0:
            ...
            for i in ready_queue:
                if t >= arrival_time[i]:
                    count = count + 1
            ready_queue[:count] = sorted(ready_queue[:count], key=lambda x: priority_val[x])
            p = ready_queue.pop(0)
            if start_time[p] == -1:
                start_time[p] = t
            completion_time[p] = t + remaining_time[p]
